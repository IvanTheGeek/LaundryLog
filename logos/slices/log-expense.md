---
slice: LogExpense → LaundryExpenseLogged
kind: CommandSlice
concern: Business
path: PATH1
status: Specified
...
- `lineTotal` is always stored — it is not recalculated at read time.
...
- Payment source (business-owned vs. personal) is implicit in the card name for now. Downstream expense report workflow is a future PATH.
